...
const Pulse = styled.div<{ layer?: number, color: string, height: number, width: number }>`
...
const Pulsating = ({ children, visible, ...other }: { children: React.ReactNode, visible: boolean, color: string, height: number, width: number }) => {
    return (
        <Wrapper>
            <Container>{children}</Container>
            {visible &&
                Array.from(Array(MAX_LAYERS).keys()).map((key) => (
                    <Pulse key={key} layer={key} color={other.color} height={other.height} width={other.width} />
                ))}
        </Wrapper>
    );
};
